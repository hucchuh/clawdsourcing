...
class TokenTraderService:
    ...
    def login(self, email: str, password: str) -> dict:
        ...
        return {"token": token, "user": {"id": row["id"], "email": row["email"], "name": row["name"]}, "expires_at": expires_at.isoformat()}
    ...
    def build_quote_for_user(self, token: str, payload: dict) -> dict:
        ...
        return {"user": user, "order": asdict(quote.order), "candidates": [asdict(x) for x in quote.candidates]}
